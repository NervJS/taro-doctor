  ...
  fn parse_error(&self, error: ValidationError) -> String {
    ...
    match error.kind {
      ...
      ValidationErrorKind::Format { format } => format!(
        r#"{} 的值 {} 不是 "{}" 格式"#,
        instance_path, error.instance, format
      ),
      ValidationErrorKind::Not { schema } => format!(
        "{} 的值 {} 不应符合 {}",
        instance_path, error.instance, schema
      ),
      ...
      ValidationErrorKind::AnyOf | ValidationErrorKind::OneOfNotValid => {
        format!("{} 的值 {} 不符合类型要求", instance_path, error.instance)
      }
      ValidationErrorKind::Contains => format!("{} 中的每一项均不属于合法的配置项", error.instance),
      ValidationErrorKind::Constant { expected_value } => {
        format!("{} 的值期望是 {}", instance_path, expected_value)
      }
      ValidationErrorKind::ContentEncoding { content_encoding } => format!(
        r#"{} 的值 {} 不符合要求的内容编码格式 "{}""#,
        instance_path, error.instance, content_encoding
      ),
      ValidationErrorKind::ContentMediaType { content_media_type } => format!(
        r#" {} 的值 {} 不符合要求的MIME 类型 "{}""#,
        instance_path, error.instance, content_media_type
      ),
      ValidationErrorKind::Enum { options } => format!(
        "{} 的值 {} 与任何指定选项 {} 都不匹配",
        instance_path, error.instance, options
      ),
      ValidationErrorKind::ExclusiveMaximum { limit } => format!(
        "{} 的值 {} 超出或等于最大值 {}",
        instance_path, error.instance, limit
      ),
      ...
      ValidationErrorKind::OneOfMultipleValid => {
        format!("{} 的值 {} 不满足要求的格式", instance_path, error.instance)
      }
      ValidationErrorKind::Pattern { pattern } => format!(
        r#"{} 的值 {} 与 "{}" 不匹配"#,
        instance_path, error.instance, pattern
      ),
      ...
      ValidationErrorKind::MultipleOf { multiple_of } => format!(
        "{} 的值 {} 不是 {}的倍数",
        instance_path, error.instance, multiple_of
      ),
      ...
      ValidationErrorKind::UniqueItems => format!(
        "{} 的值 {} 中存在重复的数组元素",
        instance_path, error.instance
      ),
      ValidationErrorKind::Type {
        kind: TypeKind::Single(type_),
      } => format!(
        r#"{} 的值 {} 不是 "{}" 类型"#,
        instance_path, error.instance, type_
      ),
      ValidationErrorKind::Type {
